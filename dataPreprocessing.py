...
class create_anotation_layer:
    ...
    def __checkCorrespondItem__(self):
        n = 0
        for image_data, anot_data in zip(self.image_path, self.anotations):
    ...
    def createAnotation(self, migrate_all_files=False):
        if self.__checkCorrespondItem__() and self.__checklen__():
            for item in tqdm(self.__imagepath_points_list):
                width = item['width']
                height = item['height']
                mask_name = f"{self.destination_path}/mask_{item['file_name']}"
                view_name = f"{self.destination_path}/view_{item['file_name']}"
...
anotate = create_anotation_layer(src_path="raw_data/test", destination_path=".")

anotate.createAnotation()